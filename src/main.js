...
const PLACEHOLDER_PATTERN = /\{\{(?:ph|placeholder):([^{}\n]+?)\}\}/g;
...
function findTokenInEditor(editor, token, sectionInfo) {
  ...
  if (sectionInfo && Number.isInteger(sectionInfo.lineStart) && Number.isInteger(sectionInfo.lineEnd)) {
    ...
    const endOffset =
      sectionInfo.lineEnd + 1 < lineCount
        ? getLineStartOffset(editor, sectionInfo.lineEnd + 1)
        : value.length;
...
const placeholderExtension = ViewPlugin.fromClass(
  class {
    constructor(view) {
...
class PlaceholderNameModal extends Modal {
...
class SnippetSuggestModal extends SuggestModal {
...
class PlaceholderSnippetsSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    this.plugin.settings.snippets.forEach((snippet, index) => {
      ...
      new Setting(section)
        ...
        .setDesc("Placeholders use {{ph:Placeholder name}}.")
